namespace DateSwipe.Client.SignalR
{
    ...
    public class SignalRService
    {
        ...
        public event Func<string, int, Task> OnMatchReceived;
        ...
        public async Task StartAsync()
        {
            ...
            _hubConnection.On<string, int>("ReceiveMatchNotification", async (message, dateId) =>
            {
                _logger.LogInformation($"Match notification received: {message} for dateId: {dateId}");
                if (OnMatchReceived != null)
                {
                    Console.WriteLine("Match");
                    await OnMatchReceived.Invoke(message, dateId);
                }
            });
